...
class Call{
    ...
    public function index(){ 
		...
		switch ($action) {
			...
			//查询所有业务
			case 'showbusiness':
				$this->showbusiness($devicenum,input('wid'));
				break;
			//查询所有业务
			case 'upwinbusiness':
				$this->upwinbusiness($devicenum,input('wid'),input('businessid'));
				break;
			default:
				echo json_encode(['data'=>array(),'code'=>'404','message'=>'未找到'],JSON_UNESCAPED_UNICODE);
				return;
    ...
    public function callheart($devicenum){
		...
		$today = date('Ymd',time());
		// 如果等待人数小于0 则直接统计排号表的等待人数
		if($count<0){
			$count = Db::name('ph_queue')->where('today',$today)->where('style',0)->whereIn('businessid',$busids)->count();
		}
	...
	public function online($userid,$online){
		...
		if(DB::name('sys_workman')->where('id',$userid)->update(['online'=>$online])){
			...
			if(!empty($lid)){
				...
				//根据设备id更改该设备的状态
				Db::name('ph_hardwareled')->where('id',$lid)->update(['online'=>$online]);
				if($online!=0){
					$time = date('Y-m-d H:i:s',time());
    				Db::name('ph_deviceqid')->insert(['online'=>$online,'ledid'=>$lid,'time'=>$time]);
				}
	...
	public function callinfo($devicenum,$userid){
		...
		$window = Db::name('sys_window')->where('id',$wid)->find();
	...
	public function call($devicenum,$userid){
		...
		if(!$wid){
			echo json_encode(['data'=>array(),'code'=>'400','message'=>'未配置窗口'],JSON_UNESCAPED_UNICODE);
			return;
		}
		...
		$map['style'] = '0';
		//根据条件查询业务范围内下一个排号id
		$que = Db::name('ph_queue')->field('id,businessid')->where($map)->order('priority,taketime')->find();
		$qid = $que['id'];
		//如果没有查找到id则返回
		if(!$qid){
	...
	public function againcall($devicenum,$id){
		...
		if(Db::name('ph_queue')->where('id',$id)->update(['style'=>'1','calltime'=>$time])){
			//添加队列到排号队列表中
			//窗口设备
			if($lid){
	...
	public function choicecall($devicenum,$id,$userid,$oldid){
		...
		if(!empty($oldid)){
			//更新当前排号的状态  完成
			Db::name('ph_queue')->where('id',$oldid)->update(['status'=>'3','style'=>'4','endtime'=>$time]);
			if(Db::name('ph_queue')->where('id',$oldid)->value('status')!='3'){
				echo json_encode(['data'=>array(),'code'=>'400','message'=>'上一个未处理'],JSON_UNESCAPED_UNICODE);
				return;
			}			
		}
	...
	/**
	 * [showbusiness 查询所有业务]
	 * @param  [string] $devicenum [呼叫器设备编号]
	 * @return [array]            [所有业务名称和id]
	 */
	public function showbusiness($devicenum,$wid){
		// $wid = Db::name('sys_window')->where('fromnum',$windownum)->value('id');
		$busid = Db::name('sys_winbusiness')->where('windowid',$wid)->value('businessid');
		$busid = explode(',',$busid);
		$list = Db::name('sys_business')->where('valid',1)->field('id,name')->select();
		foreach ($list as $k => $v) {
			if(in_array($v['id'], $busid)==true){
				$list[$k]['checked'] = 1;
			}else{
				$list[$k]['checked'] = 0;
			}
	...
	/**
	 * [upwinbusiness 修改指定窗口业务]
	 * @param  [string] $devicenum  [呼叫器设备编号]
	 * @param  [int] $wid        [窗口id]
	 * @param  [int] $businessid [业务id]
	 * @return [array]             [返回数据]
	 */
	public function upwinbusiness($devicenum,$wid,$businessid){
		if(empty($wid)||empty($businessid)){
			echo json_encode(['data'=>array(),'code'=>'400','message'=>'参数错误'],JSON_UNESCAPED_UNICODE);
			return;
		}
		// $wid = Db::name('sys_window')->where('fromnum',$windownum)->value('id');
		$businessid = $businessid.',';
		if(Db::name('sys_winbusiness')->where('windowid',$wid)->update(['businessid'=>$businessid])){
			echo json_encode(['data'=>array(),'code'=>'200','message'=>'成功'],JSON_UNESCAPED_UNICODE);
			return;
		}else{
			echo json_encode(['data'=>array(),'code'=>'400','message'=>'失败'],JSON_UNESCAPED_UNICODE);
