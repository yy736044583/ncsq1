...
class Device{
    ...
    public function index(){ 
		...
		switch ($action) {
			...
			//评价
			case 'upevaluate':
				$this->upevaluate($devicenum,input('id'),input('evaluate'),input('status'));
				break;
			//员工信息
			case 'userinfo':
			...
			// 收到截图后回复	
			case 'upstatus':
				$this->upstatus(input('id'),$devicenum);
				break;
			default:
				echo json_encode(['data'=>array(),'code'=>'404','message'=>'未找到'],JSON_UNESCAPED_UNICODE);
				return;
    ...
    public function deviceheart($devicenum){
		...
		if($down=='1'){
			echo json_encode(['data'=>['id'=>'0','type'=>$down,'photobefor'=>''],'code'=>'200','message'=>'关机'],JSON_UNESCAPED_UNICODE);
			return;
		}
		...
		switch ($led['online']) {
			case '3':
				echo json_encode(['data'=>['id'=>'0','type'=>$led['online'],'photobefor'=>''],'code'=>'200','message'=>'暂离状态'],JSON_UNESCAPED_UNICODE);
				return;
				break;
			case '4':
				echo json_encode(['data'=>['id'=>'0','type'=>$led['online'],'photobefor'=>''],'code'=>'200','message'=>'回归状态'],JSON_UNESCAPED_UNICODE);
				return;
				break;
			case '5':
				echo json_encode(['data'=>['id'=>'0','type'=>$led['online'],'photobefor'=>''],'code'=>'200','message'=>'登陆状态'],JSON_UNESCAPED_UNICODE);
				return;
				break;
			default:break;
		}
		$list = Db::name('pj_evaluate')
		->where("workmanid='$userid' and evaluatestatus=0")
		->order('id')->find();
		//查询发起评价的id
		$eid = $list['id'];
		if(!$eid){
			$arr = ['id'=>0,'type'=>0,'photobefor'=>''];
			echo json_encode(['data'=>$arr,'code'=>'200','message'=>'无评价'],JSON_UNESCAPED_UNICODE);
			return;
		}

		//如果截图为1则回掉图片地址  否则回掉该ID
		if($list['evaluatetype']==1){
			//获取当前域名
			$request = request();
			$path1 = $request->domain().dirname($_SERVER['SCRIPT_NAME']).'/public/uploads/';
			$arr = ['id'=>"$eid",'photobefor'=>$path1.$list['photobefor'],'type'=>'6'];
			echo json_encode(['data'=>$arr,'code'=>'200','message'=>'截图'],JSON_UNESCAPED_UNICODE);
			return;
		}	
     ...
     * @param  [type] $id        [评价id]
     * @param  [type] $status    [评价状态 2评价取消/截图错误，3评价超时/未确认截图，4等待评价器显示/未评价]
     * @return [type]            [返回id]
     */
     ...
     * @param  [int]  $status  [1签字截图 0评价]
     * @return [array]  data     [返回数据集]
     */
    public function upevaluate($devicenum,$id,$evaluate,$status){
    	if(empty($id)){
			echo json_encode(['data'=>array(),'code'=>'400','message'=>'参数错误'],JSON_UNESCAPED_UNICODE);
			return;	
    	}
    	if($status=='1'){
    		$path =  ROOT_PATH . 'public' . DS . 'uploads'.DS.'picture'; // 接收文件目录
	        if (!file_exists($path)) {
	            if(!mkdir($path)){
	                echo '提交失败,自动创建文件夹失败';
	            }
	    	...
	    	if($_FILES['file']['name'] != ""){
				...
				$url = $path.DS.time().$id.'.png';
				copy($_FILES['file']['tmp_name'],$url)  or die ("Could not copy file"); 

				$url1 = 'picture'.DS.time().$id.'.png';
			
				$data1['evaluatetime'] = date('Y-m-d H:i:s',time());
				$data1['photoautograph'] = $url1;
				$data1['status'] = '2'; //已确认截图
				$data1['evaluatestatus'] = '1'; //成功
				//更新截图到数据库  
				if(DB::name('pj_evaluate')->where('id',$id)->update($data1)){
					$data = array('code'=>'200','data'=>['id'=>$id,'status'=>'1'],'message'=>'成功');
				}else{
					$data = array('code'=>'200','data'=>['id'=>$id,'status'=>'0'],'message'=>'成功');
				}
				echo json_encode($data,JSON_UNESCAPED_UNICODE);
				return;
			}else{
				echo json_encode(['data'=>['id'=>$id,'status'=>'0'],'code'=>'400','message'=>'未上传文件'],JSON_UNESCAPED_UNICODE);
				return;
			}
	...
	//成功接收之后回掉更新数据库 表示已接收到数据
	public function upstatus($id,$DeviceNumber){
		$did = Db::name('pj_device')->where('number',$DeviceNumber)->value('id');
		
		if(Db::name('pj_evaluate')->where('id',$id)->update(['evaluatestatus'=>'5','deviceid'=>$did])){
			$data = array('code'=>'200','data'=>array(),'msg'=>'成功');
		}else{
			$data = array('code'=>'400','data'=>array(),'msg'=>'失败');
		}
		echo json_encode($data,JSON_UNESCAPED_UNICODE);
	}
