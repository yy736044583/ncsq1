...
class Cled{
    ...
    public function cledheart($devicenum){
		...
		//$cled = Db::name('ph_cled')->field('usestatus,id')->where('number',$devicenum)->find();
    ...
    public function showled($devicenum,$id){
		...
		$data['windowname'] = Db::name('sys_window')->where('id',$list['windowid'])->where('valid',1)->value('fromnum');
		$data['id'] = $list['id'];
		echo json_encode(['data'=>$data,'code'=>'200','message'=>'正常'],JSON_UNESCAPED_UNICODE);
		return;
    }
    ...
    public function bannerinfo($devicenum){
    	// 查询视频地址
    	$redio = Db::name('ph_banner')->where('type',2)->where('top',1)->value('url');
    	// 查询轮播地址
