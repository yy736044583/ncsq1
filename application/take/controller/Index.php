...
class Index extends \think\Controller{
    ...
    public function business(){
        ...
        foreach ($list as $key => $value) {
            $arrayselce = array('businessid' => $list[$key]['id'], 'today' => date('Ymd'));
    ...
    public function business1(){
        ...
        $cond['id'] = array('in',$businessids);
        $cond['valid'] = 1;
        $info = Db::table('sys_business')->field('name,id,fromdescribe,waitcount,day,maxnumber,startnumber,maxnumberam')->where($cond)->order('sort')->paginate(6,true,['query'=>array('devicenum'=>$number)]);
        // dump($info);die;
        $list = $info->all();
        // 获取默认排队人数
        $setup = Db::name('ph_setup')->where('id',1)->find();
        ...
        foreach ($list as $key => $value) {
           ...
           // 如果该业务没有设置排队人数则使用默认排队人数
            ...
            $list[$key]['fromdescribe']  = explode(',',$value['fromdescribe']);
    ...
    public function business3(){
        ...
        $cond['id'] = array('in',$businessids);
        $cond['valid'] = 1;
        $info = Db::table('sys_business')->field('name,id,fromdescribe,waitcount,day,maxnumber,startnumber,maxnumberam')->where($cond)->order('sort')->paginate(9,true,['query'=>array('devicenum'=>$number)]);
        // dump($info);die;
        $list = $info->all();
        // 获取默认排队人数
        $setup = Db::name('ph_setup')->where('id',1)->find();
    ...
    public function business4(){
        ...
        $cond['id'] = array('in',$businessids);
        $cond['valid'] = 1;
        $info = Db::table('sys_business')->field('name,id,fromdescribe,waitcount,day,maxnumber,startnumber,maxnumberam')->where($cond)->order('sort')->paginate(8,true,['query'=>array('devicenum'=>$number)]);
        // dump($info);die;
        $list = $info->all();
        // 获取默认排队人数
        $setup = Db::name('ph_setup')->where('id',1)->find();
    ...
    //获取排队人数
    public function people_num(){
        $id = input('id');
        $today = date('d',time());
        // 查询业务信息 以及上下午的最大人数
        $bus = Db::table('sys_business')->where('id',$id)->where('day',$today)->find();
        // 默认最大人数
        $setup = Db::name('ph_setup')->where('id',1)->field('maxnumber,maxnumberam')->find();
        $work = date('H:i:s',time());
        // 查询工作时间
        $worktime = Db::name('sys_thiscenter')->where('id',1)->field('worktime_e_am,worktime_s_am,worktime_e_pm,worktime_s_pm')->find();
        ...
        if($work>$worktime['worktime_s_pm']&&$work<$worktime['worktime_e_pm']){
            ...
            $maxnumber = $bus['maxnumberam'];
            $setnumber = $setup['maxnumberam'];
        }elseif($work>$worktime['worktime_e_pm']){
            // 下午取号时间 进行显示
            $timefine = date('H:i',strtotime($worktime['worktime_s_pm']));
            // type 1非工作时间 2业务已满号 3正常
            echo json_encode(['type'=>1,'waitcount'=>$waitcount,'timefine'=>$timefine]);
            return;
        }else{
             // 下午取号时间 进行显示
            $timefine = date('H:i',strtotime($worktime['worktime_s_pm']));
        ...
        $arrayselce = array('businessid' => $id, 'today' => date('Ymd'));
        // 查询当天取号人数
        $counts = Db::table('ph_queue')->where($arrayselce)->count();
        // 获取取号最大人数
        $maxnumber = !empty($maxnumber)?$maxnumber:$setnumber;

        // 如果大于最大人数返回空
        if($counts>=$maxnumber){
            echo json_encode(['type'=>2,'waitcount'=>$waitcount]);
            return;
        }else{
            echo json_encode(['type'=>3,'waitcount'=>$waitcount]);
            return;
        }
