...
**  硬件呼叫器管理 
 */
class Hardwarecall extends Common{
	public function index(){
		...
		if($usestatus!=''|$number!=''|$windowid!=''){
			...
			$data = DB::name('ph_hardwarecall')->where($map)->order('createtime desc')->paginate(12,false,['query'=>array('windowid'=>$windowid,'number'=>$number,'usestatus'=>$usestatus)]);
		}else{
			$data = DB::name('ph_hardwarecall')->order('createtime desc')->paginate(12);
		}
	...
	public function addcall(){
		if(request()->isPost()){
			...
			if($validate->check($data)){
				if(Db::name('ph_hardwarecall')->insert($data)){
					$this->success('添加成功','hardwarecall/index');	
				}else{
					$this->error('添加失败,请重试');
				}				
			}else{
	...
	public function upcall(){
		if(request()->isPost()){
			...
			$validate = validate('Device');
			if($validate->check($data)){
				if(Db::name('ph_hardwarecall')->where('id',$lid)->update($data)){
					$this->success('修改成功','hardwarecall/index');	
				}else{
					$this->error('修改失败,请重试');
				}
			}else{
		...
		$id = input('id');
		$list = DB::name('ph_hardwarecall')->where('id',$id)->find();
		$window = DB::name('sys_window')->where('valid',1)->select();
		$center = Db::name('ph_hardwarecenter')->where('usestatus',1)->select();
		$this->assign('center',$center);
		$this->assign('win',$window);
	...
	public function dlcall(){
		$id = input('id');
		if(Db::name('ph_hardwarecall')->where('id',$id)->delete()){
			$this->success('删除成功','hardwarecall/index');
		}else{
			$this->error('删除失败');
		}
	}
	...
	public function showtype(){
		...
		$list = Db::name('ph_hardwarecall')->field('id,lastlogin,number')->where('usestatus',1)->select();
		$nowtime = time();
		$data = array();
		foreach ($list as $k => $v) {
