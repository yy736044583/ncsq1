...
function OpenDevice(value){
    ...
    if(Capture.StartDevice(szDeviceIndex) == 0)
	{
		...
		FillInRes();
		/*if (szDeviceIndex == "0") {
		    Capture.SetResolution(szDeviceIndex, "2592", "1944");
		}
		else if (szDeviceIndex == "1" || szDeviceIndex == "2") {
		    Capture.SetResolution(szDeviceIndex, "1600", "1200");
		}*/
	}
	else
	{
...
{	
	...
	//Capture.GetResolution(szDeviceIndex);
	//alert(Capture.GetResolution(szDeviceIndex));
    //var strResolutions = JSON.parse('{resolution:["1*2","3*4"]}');
	...
	if(szPostfix != ".pdf" && szPostfix != ".tif")
	{
		...
		if(strFileNames.length>0){
			...
			for(var i=0;i<strFileNames.length;i++){
				var src = 'data:image/gif;base64,' + strFileNames;
				var Html = "<li><img src='"+src+"' data-id='"+i+"'></li>";
...
function FaceVertify()
{
	...
	Preview(1);
	//ShowPreviewBase64(, 0);
	Capture.CaptureImage(szDeviceIndex,"d:\\DocImage\\face.jpg");
	strFileNames.push(Capture.EncodeBase64("d:\\DocImage\\face.jpg"));
	Preview(1);
	//ShowPreviewBase64(Capture.EncodeBase64("d:\\DocImage\\face.jpg"), 1);
	var iResult = Capture.FaceVertify("d:\\DocImage\\IDCard.jpg", "d:\\DocImage\\face.jpg");
